#!/usr/bin/env python3
"""
Plot computation and communication cost for the DP tree mechanism
as a function of the number of clients (NPARTS).

This script wraps the notebook logic in a reusable CLI tool.

Example:
  python scripts/boolean_circuit/plot_tree_mech_cost_estimate.py --nparts 32 64 128 256 --vec-dim 1
"""
...
def _run_one(args: Tuple[int, int, str, str, str]) -> Tuple[float, float, float, float]:
    """
    Worker function for parallel cost computation for the tree mechanism.

    Runs in a separate process when using ProcessPoolExecutor.
    """
    nparts, vec_dim, project_dir, scripts_dir, data_dir = args
...
def plot_tree_mech_costs(
    ...
    worker_args = [(nparts, vec_dim, project_dir, scripts_dir, data_dir) for nparts in nparts_list]
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Plot DP tree mechanism cost vs number of clients (NPARTS)."
    )
    parser.add_argument(
        "--nparts",
        nargs="+",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Vector dimension D for the tree mechanism (default: 1).",
    )
    parser.add_argument(
        "--data-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="tree_mech",
        help="Base circuit name (default: tree_mech).",
    )
